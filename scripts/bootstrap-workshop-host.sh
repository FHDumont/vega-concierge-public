#!/usr/bin/env bash
# Bootstrap idempotente — host Ubuntu para workshop Vega (EC2 / VM bare metal).
# Instala deps de host, grava .env, systemd + pull GHCR + stack via up.sh.
#
# Pré-requisito: clone deste repo (ex.: /opt/vega-concierge).
#
...
#
# Opções:
#   --control-password PASS   senha do terminal Ops (default: vega-workshop)
#   --deployment-env NAME     DEPLOYMENT_ENVIRONMENT (default: user-<hostname>)
#   --repo-dir PATH           raiz do repo (default: pai de scripts/)
#   --repo-owner USER         dono dos arquivos do repo (default: SUDO_USER ou ubuntu)
#   --hugo-version VER        Hugo extended .deb (default: 0.164.0)
#   --skip-models             não roda ollama pull
#   --skip-up                 só host + systemd; não roda up.sh
#   --force-env               regrava .env mesmo se existir
#   --check                   só verifica (não instala)
#   -h, --help
#
...
if [[ "$(id -u)" -ne 0 ]]; then
  die "execute com sudo (root)."
fi

if [[ "$(uname -m)" != "x86_64" ]]; then
  die "suporta amd64 (x86_64) apenas — arm64 (Mac mini): deps manuais + up.sh (deploy-pull-only.md)."
fi

if [[ ! -f "$REPO_DIR/scripts/up.sh" || ! -f "$REPO_DIR/.env.example" ]]; then
  die "repo inválido em $REPO_DIR — falta scripts/up.sh ou .env.example"
fi
...
log "início — repo=$REPO_DIR owner=$REPO_OWNER env=$DEPLOYMENT_ENV"
...
# ttyd do apt habilita unit padrão — conflita com vega-ttyd na :7681
...
log "Hugo extended ${HUGO_VERSION} (.deb — não snap; snap não lê /opt)…"
...
# Gerado por scripts/bootstrap-workshop-host.sh ($(date -u +%Y-%m-%d))
...
log "workshop: permissões Hugo (evita public/ root-owned)…"
...
run_check
log "concluído — loja :3000 · API :8000 · Ops :9000 · guia :1313"
