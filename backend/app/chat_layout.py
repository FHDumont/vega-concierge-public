...
from .store.catalog_format import (
...
def policy_sections_from_chunks(
    ...
    *,
    ...
    for chunk in chunks or []:
        ...
        dedupe = f"{chunk.get('source', '')}:{title}"
        if dedupe in seen:
            continue
...
def shopper_reply_from_layout(layout: dict[str, Any] | None, fallback: str) -> str:
    """Short bubble text when structured layout carries the detail."""
    text = (fallback or "").strip()
    if not layout:
        return text
    if layout.get("sections") or layout.get("facts") or layout.get("bullets"):
        return (layout.get("lead") or text).strip()
    return text
...
def build_store_chat_layout(
    ...
    *,
    ...
    sections = (
        policy_sections_from_chunks(chunks or [], body_max_len=320 if overview else 480)
        if chunks else []
    )
    if not sections:
    ...
    if not overview and topic:
        ...
        expected_source = _TOPIC_POLICY_SOURCE.get(topic)
        topic_chunks = [
            chunk for chunk in (chunks or [])
            if not expected_source or chunk.get("source") == expected_source
        ] or list(chunks or [])
        topic_sections = policy_sections_from_chunks(topic_chunks, body_max_len=480)
        if len(topic_sections) >= 2:
            return {
                "lead": _first_sentence(answer) or f"Here's how {topic_title.lower()} work:",
                "sections": topic_sections[:6],
            }
        sentences = _split_sentences(answer)
        if len(sentences) >= 2:
            return {
        ...
        return {
            "lead": f"Here's how {topic_title.lower()} work:",
            "sections": [{"title": topic_title, "body": answer}],
        }

    if len(sections) >= 2:
        return {"lead": _first_sentence(answer) or "Here's what our policies say:", "sections": sections[:5]}

    if len(sections) == 1 and len(answer) > 60:
        return {"lead": _first_sentence(answer) or sections[0]["title"], "sections": sections}
