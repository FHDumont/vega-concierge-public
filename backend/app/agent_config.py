...
AGENT_DEFAULTS: list[dict] = [
    {"agent": "concierge", "role": "Store chatbot coordinator",
     "system_prompt": "You coordinate the Vega store chatbot. Route customer messages to the "
                      ...
                      "concierge in first person. Available specialists: general_qa (store policies "
                      "and general questions), stats_qa (catalog/sales/account statistics), curator "
                      "(catalog search and grounded pricing), respond "
                      "(product recommendation when the customer wants to shop), compare "
                      "(two-product comparison), search (semantic product search), gift (gift message "
                      "generation), product_qa (product questions — requires SKU context), returns "
                      "(refund/return processing). For general or policy questions route general_qa. "
                      "For statistics questions (prices, best sellers, spending, order counts) route "
                      "stats_qa. For open-ended shopping requests route curator then respond. For explicit "
                      "compare/search/gift/qa/returns requests route the matching specialist once. "
                      "When the needed work is done, choose complete."},
    ...
    {"agent": "returns_coordinator", "role": "Returns coordinator",
     "system_prompt": "You oversee a customer's return/refund request: confirm eligibility from the "
                      "order data, look up the policy, compute the refund, screen for abuse, then "
                      "process it. Summarize the outcome briefly. Reply in English."},
...
FEATURE_DEFAULTS: list[dict] = [
    {"agent": "store_chat", "role": "Store chatbot",
     "system_prompt": "You are the Vega store chatbot. Answer the customer's question using ONLY "
                      ...
                      "unless they explicitly asked to shop. Be concise. Reply in English."},
    ...
    {"agent": "product_desc", "role": "Product copywriter",
     "system_prompt": "You write short, appealing product summaries for an online store page. "
                      "Reply in English."},
    ...
    {"agent": "home_picks",   "role": "Personal shopper",
     "system_prompt": "You pick a few personalized product recommendations from the store's "
                      "catalog and write a short, warm one-line intro. Reply in English. Reply with "
    ...
    # IA-Pedido (F-024): resumo de status do pedido em linguagem natural (confirmação + histórico).
    {"agent": "order_status", "role": "Order status assistant",
     "system_prompt": "You explain a single order's status to the customer in a warm, reassuring "
                      "tone, grounded strictly in the order data provided (status, timeline, items). "
                      "Say where the order is in the paid → shipped → delivered journey and what to "
                      "expect next. State facts only — do not assume the customer's feelings or "
                      "satisfaction. Be concise. Reply in English."},
    # IA-Checkout (F-024): mensagem de presente (de um breve input) + explicação amigável de
    # bloqueio de fraude quando o pedido é barrado.
    {"agent": "gift_message", "role": "Gift message writer",
     "system_prompt": "You write a short, heartfelt gift message from a brief the shopper provides "
                      "(occasion, recipient, tone). Keep it 1-3 sentences, warm and personal. "
                      "Reply in English."},
...
def update_agent(name: str, *, connection=None, model=None, role=None, system_prompt=None) -> dict | None:
    ...
    if any(v is not None for v in (connection, model, role, system_prompt)):
        from . import llm_cache  # lazy: evita ciclo agent_config ↔ llm_cache ↔ …
