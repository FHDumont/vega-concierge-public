...
from ..tool_arg_normalize import (
    ...
    normalize_policy_lookup_args,
    normalize_refund_calc_args,
    normalize_search_catalog_args,
    normalize_search_policies_args,
    normalize_sku_arg,
)
...
class GetPriceInput(BaseModel):
    ...
    sku: Any = Field(
        ...
        description="Product SKU from the catalog (e.g. NS-001). Pass exactly one SKU — call "
                    "get_price once per SKU, never a batch.",
    )
...
class PolicyLookupInput(BaseModel):
    ...
    status: Any = Field(
        default=None, description="Order lifecycle status (e.g. DELIVERED, SHIPPED).",
    )
    total: Any = Field(default=None, description="Order total amount in BRL.")
...
class SearchPoliciesInput(BaseModel):
    ...
    question: str | None = Field(
        default=None,
        description="Customer question about a store policy (returns, shipping, warranty, payment).",
    )
...
def _policy_lookup_tool(order_id: Any = None, status: Any = None, total: Any = None, **kwargs):
    norm_status, err = normalize_policy_lookup_args({"status": status, **kwargs})
    if err:
        return err
    return policy_lookup({"order_id": order_id, "status": norm_status, "total": total})
...
def _search_policies_tool(question: str | None = None, config: RunnableConfig = None, **kwargs):
    ...
    if err:
        return err
    return search_policies(q, config=config)


def _get_catalog_stats_tool(*, config: RunnableConfig = None, **kwargs):
    """Aggregate catalog price/stock facts for stats chat."""
    from ..ai_agents.stats_chat import catalog_stats
...
def _get_account_stats_tool(*, config: RunnableConfig = None, **kwargs):
    """Signed-in shopper account aggregates for stats chat (spend, orders, tier)."""
    from ..ai_agents.stats_chat import account_stats

    del kwargs
    meta = (config or {}).get("metadata") if config else {}
...
def _refund_calc_tool(order_id: Any = None, status: Any = None, total: Any = None, **kwargs):
    norm_total, err = normalize_refund_calc_args({"total": total, **kwargs})
    if err:
        return err
    return refund_calc({"order_id": order_id, "status": status, "total": norm_total})
...
def _check_refund_eligibility_tool(order_json: str, config: RunnableConfig) -> str:
    """Compatibility tool with the historical response shape, owned by store rules."""
    del config
    try:
    ...
    result = {
        ...
        "llm_eligible": None,
        "reason": "Only delivered orders can be refunded." if order.get("status") != "DELIVERED" else "Delivered order.",
        "llm_response": "",
        "source": "store_rule",
    }
...
def _decide_fraud_allow_or_block_tool(
    ...
    """Compatibility tool that delegates to the isolated fulfillment boundary."""
    try:
    ...
    if not isinstance(quote, dict):
        quote = {}
    from ..ai_agents.fulfillment_workflow import _decide_fraud_allow_or_block_tool

    result = json.loads(_decide_fraud_allow_or_block_tool(json.dumps(quote), total, config))
    return json.dumps(result)
...
def _delete_product_tool(sku: Any = None, config: RunnableConfig = None) -> str:
    ...
    def _compute() -> dict:
        return delete_product(norm_sku)

    result = galileo_control.controlled_delete_product(norm_sku, _compute)
    return json.dumps(result)
...
get_catalog_stats_tool = StructuredTool.from_function(
    func=_get_catalog_stats_tool,
    name="get_catalog_stats",
    description=(
        "Return aggregate catalog statistics (product count, cheapest, most expensive, price range). "
        "Use for shopper questions about prices and catalog extremes."
    ),
)

get_account_stats_tool = StructuredTool.from_function(
    func=_get_account_stats_tool,
    name="get_account_stats",
    description=(
        "Return the signed-in shopper's account statistics (total spend, order count, tier). "
        "Use for personal spending and purchase-history questions."
    ),
)
